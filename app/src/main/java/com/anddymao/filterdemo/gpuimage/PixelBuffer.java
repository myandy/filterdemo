package com.anddymao.filterdemo.gpuimage;
...
public class PixelBuffer {
    ...
    public Bitmap getBitmap(Bitmap bitmap) {
        ...
        // Call the renderer draw routine (it seems that some mFilters do not
...
//        mRenderer.onDrawFrame(mGL);
        ...
        try {
            ...
            Log.e(TAG, "convertToBitmap error:" + e.toString());
            return null;
        } catch (Exception e) {
            Log.e(TAG, "convertToBitmap error:" + e.toString());
        }
...
//        mRenderer.onDrawFrame(mGL);
    ...
    /**
     * @param recyclerBitmap 复用bitmap,为null才新建bitmap
     */
    private void convertToBitmap(Bitmap recyclerBitmap) {
        if (recyclerBitmap != null && !recyclerBitmap.isRecycled() && recyclerBitmap.getConfig() == Bitmap.Config.ARGB_8888) {
            mBitmap = recyclerBitmap;
        } else {
            mBitmap = Bitmap.createBitmap(mWidth, mHeight, Bitmap.Config.ARGB_8888);
        }
        ByteBuffer ib = ByteBuffer.allocate(mBitmap.getByteCount());
